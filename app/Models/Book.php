...
class Book extends Model
{
    ...
    public function scopeWithAvailableCopies(Builder $query): Builder
    {
        return $query->selectSub(function ($subQuery): void {
            $subQuery->from('Copies')
                ->selectRaw('COUNT(*)')
                ->whereColumn('Copies.Bid', 'Books.Bid')
                ->where('Copies.Status', 'Available')
                ->whereNotExists(function ($activeBorrows): void {
                    $activeBorrows->from('Borrows')
    ...
    public function scopeWithAuthorNames(Builder $query): Builder
    {
        return $query->selectSub(function ($subQuery): void {
            $subQuery->from('BookAuthor')
    ...
    public function scopeSearchByTitleOrAuthor(Builder $query, ?string $search = null): Builder
    {
        if (!$search) {
            return $query;
        }

        return $query->where(function ($inner) use ($search): void {
